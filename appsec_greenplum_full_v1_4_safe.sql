...
-- Версия 1.4 SAFE: только целевая схема; без создания схемы и без DROP ... CASCADE
...
-- Защитные проверки: только нужная БД и уже существующая целевая схема.
DO LANGUAGE plpgsql $$
BEGIN
...
-- Очищается только производная таблица в целевой схеме.
...
historical_target_dates AS (
    SELECT
        i.product_id,

        MAX(
            CASE
                WHEN d.dt <= CURRENT_DATE - 28
                THEN d.dt
                ELSE NULL
            END
        ) AS target_dt_28,
...
historical_points AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN d.dt = t.target_dt_28
                THEN d.debt_remaining
                ELSE NULL
            END
        )::INTEGER AS remaining_28d_ago,

        MAX(
            CASE
                WHEN d.dt = t.target_dt_90
                THEN d.debt_remaining
                ELSE NULL
            END
        )::INTEGER AS remaining_90d_ago

    FROM historical_target_dates t
    LEFT JOIN daily_product_debt d
